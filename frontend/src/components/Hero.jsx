...
const slides = [
  {
    ...
    title: "Ngeme: The Sun of the Sea",
...
export default function Hero() {
  ...
  const prevSlide = () => {
    ...
    setIsAuto(false); // Stop auto-play on manual interaction
  };
  ...
  return (
    <section className="relative h-screen min-h-[750px] overflow-hidden bg-volcanic">
      ...
      {/* Background Layer - The Crossfade logic */}
        ...
        <AnimatePresence mode="popLayout">
          ...
          >
            <motion.img 
              ...
              alt="Ngeme Resort" 
            />
            {/* Soft, professional shadow overlay (No pitch black) */}
            <div className="absolute inset-0 bg-gradient-to-b from-black/50 via-transparent to-black/60" />
            <div className="absolute inset-0 bg-ocean/10 mix-blend-overlay" />
          </motion.div>
        </AnimatePresence>
      </div>

      {/* Content Layer */}
      <div className="relative z-20 h-full container flex flex-col items-center justify-center text-center px-6">
        ...
        <motion.span
          ...
          className="text-lush font-bold tracking-[0.5em] uppercase text-xs mb-6 drop-shadow-lg"
        >
        ...
        <motion.h1 
          ...
          animate={{ opacity: 1, y: 0 }}
          className="text-6xl md:text-[10rem] font-serif leading-none mb-8 text-white drop-shadow-[0_5px_15px_rgba(0,0,0,0.4)]"
        >
        ...
        <motion.p 
          ...
          className="text-lg md:text-2xl max-w-3xl font-light italic text-white/90 mb-12 drop-shadow-md"
        >
        ...
        <div className="flex flex-col sm:flex-row gap-6 relative z-30">
          <button className="btn-primary !bg-white !text-ocean border-none shadow-2xl hover:!bg-lush hover:!text-white group">
            Make a Reservation
            <span className="ml-2 group-hover:translate-x-1 transition-transform inline-block">→</span>
          </button>
          <button className="btn-outline border-white/40 text-white backdrop-blur-md hover:bg-white/10">
            The Foundation Story
          </button>
        </div>
      </div>

      {/* Manual Switch Controls (The Professional "Bad Ass" look) */}
      <div className="absolute inset-x-0 bottom-32 z-40 flex justify-between container pointer-events-none px-10">
        <button 
          onClick={prevSlide}
          className="pointer-events-auto w-16 h-16 rounded-full border border-white/20 flex items-center justify-center text-white backdrop-blur-sm hover:bg-white hover:text-ocean transition-all group"
        >
          <svg viewBox="0 0 24 24" className="w-6 h-6 fill-none stroke-current stroke-2 group-hover:-translate-x-1 transition-transform">
        ...
        <div className="flex items-center gap-4">
          {slides.map((_, i) => (
            <button 
              ...
              className={`h-1 transition-all duration-700 pointer-events-auto ${current === i ? "w-16 bg-lush" : "w-6 bg-white/30 hover:bg-white/60"}`} 
            />
          ))}
        </div>
      ...
      {/* Trust Badges - Static and subtle */}
